...
namespace rannc {
    ...
    std::vector<K> keys(const std::unordered_map<K, V, H>& map, bool sorted=true) {
        std::vector<K> keys;
    ...
    template <typename K, typename V>
    std::string join_map_as_str(const std::unordered_map<K, V> &map, const char *delim = ",", const int maxlen = 0) {
...
namespace rannc {
    ...
    std::vector<O> flatMap(const std::vector<I> &vec, std::function<std::vector<O>(I)> f) {
    ...
    template<typename K, typename V>
    std::unordered_map<K, V> addAll(const std::unordered_map<K, V>& v1, const std::unordered_map<K, V>& v2) {
    ...
    template <class T>
    struct EnumHash {
        static_assert(std::is_enum<T>::value, "This hash only works for enumeration types");
