...
inline IValue _createGenericDict(
    const py::dict& obj, const TypePtr& key_type, const TypePtr& value_type) {
...
IValue _toIValue(
    py::handle obj, const TypePtr& type, c10::optional<int32_t> N) {
  switch (type->kind()) {
    ...
    case TypeKind::DictType: {
      ...
      return _createGenericDict(
          py::cast<py::dict>(obj), dict_type->getKeyType(),
    ...
    case TypeKind::ClassType: {
      ...
      for (size_t slot = 0; slot < numAttrs; slot++) {
        ...
        try {
          ...
          throw py::cast_error(c10::str(
              "Could not cast attribute '", attrName, "' to type ",
              attrType->repr_str(), ": ", e.what()));
        }
      }
      return userObj;
    }
    case TypeKind::InterfaceType: {
      ...
      if (auto mod = as_module(py::cast<py::object>(obj))) {
        ...
        if (!classType) {
          throw std::runtime_error(c10::str(
              "Assigning the object ", py::str(obj),
      ...
      if (!classType->isSubtypeOfExt(interfaceType, &why_not)) {
        throw py::cast_error(c10::str(
            "Object ", py::str(obj), " is not compatible with interface ",
    ...
    case TypeKind::NumberType: {
      ...
      #ifdef USE_RPC
                          return
      obj.cast<torch::distributed::rpc::PyRRef>().toIValue(); #else
                          AT_ERROR("RRef is only supported with the distributed
      package"); #endif } break; case TypeKind::PyObjectType: { return
      c10::ivalue::ConcretePyObjectHolder::create(obj);
                      }
                      case TypeKind::CapsuleType: {
                          return
      IValue::make_capsule(py::cast<c10::Capsule>(obj).obj_ptr);
                      }
                      case TypeKind::FutureType: {
                          return
      obj.cast<std::shared_ptr<PythonFutureWrapper>>()->fut;
                      }
                      ...
                      case TypeKind::EnumType:
                          ...
                          py::object py_obj =
      py::reinterpret_borrow<py::object>(obj); std::string name =
      py::cast<std::string>(obj.attr("name")); IValue value =
      _toIValue(obj.attr("value"), enum_type->getValueType(), {}); auto
      enum_holder = c10::make_intrusive<c10::ivalue::EnumHolder>(enum_type,
      name, value); return IValue(enum_holder);
                          */
  }
  throw py::cast_error(c10::str(
