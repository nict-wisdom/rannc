...
namespace rannc {
    ...
    int getCurrentRank() {
        if (with_rank_stack.empty()) return 0;
        return with_rank_stack.back();
    }


    torch::jit::TypedIValue tranformTensorsInTypedIValue(const torch::jit::TypedIValue &typed_ivalue,
...
//            std::vector<torch::jit::IValue> trans_elems;
            ...
            for (size_t i=0; i<elems.size(); i++) {
                auto tiv = tranformTensorsInTypedIValue({elems.at(i), elem_types.at(i)}, f, tf);
            ...
            return {c10::ivalue::Tuple::create(trans_elems), c10::TupleType::create(trans_types)};
        }
        return {ivalue, type};
    }
//
...
//            return torch::jit::CompleteTensorType::create(at::ScalarType::Float, ctt->device(), ctt->sizes());
//        }
//        return type;
//    }
//
//    torch::jit::TypedIValue toFloatTensorsInTypedIValue(const torch::jit::TypedIValue &ivalue) {
//        return tranformTensorsInTypedIValue(ivalue, toFloatIfHalf, toFloatTypeIfHalfType);
//    }
//
    torch::jit::TypedIValue sliceBatchTensorsInTypedIValue(const torch::jit::TypedIValue &ivalue, int offset, int count) {
        return tranformTensorsInTypedIValue(ivalue, [offset, count](const at::Tensor& t) {
           ...
           [count](const torch::jit::TypePtr& type) {
               ...
               return torch::jit::TensorType::create(ctt->scalarType(), ctt->device(), ctt->sizes(), ctt->strides(), c10::nullopt);
            });
    }

    std::vector<std::pair<std::string, at::Tensor>> toNamedTensors(const std::vector<py::tuple> &pytuple_tensors) {
        std::vector<std::pair<std::string, at::Tensor>> named_tensors;
        for (const py::tuple& p: pytuple_tensors) {
            named_tensors.emplace_back(py::cast<std::string>(p[0]), py::cast<at::Tensor>(p[1]));
        }
        return named_tensors;
    }

    std::vector<at::Tensor> toTensors(const std::vector<py::tuple> &params,
    ...
    static bool CompareIValue(const std::pair<torch::jit::IValue, torch::jit::IValue>& aWrap,
    ...
    std::vector<std::pair<torch::jit::IValue, torch::jit::IValue>> iterationOrder(const c10::Dict<torch::jit::IValue, torch::jit::IValue>& dict) {
    ...
    static torch::jit::IValue addInput(const std::shared_ptr<torch::jit::tracer::TracingState> & state, const torch::jit::IValue& input,
            const torch::jit::TypePtr& type, torch::jit::Value* value) {
        value->setType(type);
        if (type->isSubtypeOf(torch::jit::TensorType::get())) {
            ...
            AT_ERROR(
                    ...
                    "inputs to traced functions. Got ", type->repr_str());
        }
    }

    std::shared_ptr<torch::jit::tracer::TracingState> _trace(
        torch::jit::Stack inputs,
        c10::TupleTypePtr ttp,
...
//        }
        ...
        for (const torch::jit::NameValue& s : self->named_attributes(/*recurse=*/false)) {
        ...
        torch::jit::tracer::getTracingState()->lookup_var_name_fn = std::move(var_name_lookup_fn);
...
//        }
    ...
    std::shared_ptr<torch::jit::Graph> _createGraphByTracing(
        ...
        auto lookup_fn_adapter = [var_name_lookup_fn](const Variable& var) -> std::string {
            ...
            if (rank == 0) {
                ...
                ss << "r" << getCurrentRank() << "_" << py::cast<std::string>(var_name_lookup_fn(var));
            }
            return ss.str();
        };
    ...
    void stackParams(std::vector<torch::jit::IValue>& param_ivalues,
    ...
    std::shared_ptr<torch::jit::Graph> trace(const py::tuple& input_tuple,
        ...
        std::shared_ptr<torch::jit::script::Module> module = std::make_shared<torch::jit::script::Module>("dummy");
        ...
        torch::jit::TypedIValue info = sliceBatchTensorsInTypedIValue({iv, iv.type()}, 0, batch_size);
