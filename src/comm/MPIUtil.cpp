...
//// The above returns different values in different ranks.
    ...
    std::unordered_set<int> getRanksInComm(MPI_Comm comm) {
        ...
        MPI_Group_translate_ranks(grp, size, &grp_ranks[0], world_grp, &translated_ranks[0]);
    ...
    int64_t allReduceSumBatchSize(int64_t batch_size, MPI_Comm comm) {
        int64_t sum = 0;
        mpi::checkMPIResult(MPI_Allreduce(&batch_size, &sum, 1, MPI_LONG, MPI_SUM, comm));
        return sum;
    }

    int64_t allReduceMaxBatchSize(int64_t batch_size, MPI_Comm comm) {
        int64_t sum = 0;
        mpi::checkMPIResult(MPI_Allreduce(&batch_size, &sum, 1, MPI_LONG, MPI_MAX, comm));
        return sum;
    }
}
