...
struct RouteDP {
  RouteDP() = default;
  RouteDP(
      IValueLocation location, std::vector<int> sources, std::vector<int> dests,
      int tag, int source_tag, RouteTypeDP type)
  ...
  RouteDP(
      IValueLocation location, std::vector<int> sources, std::vector<int> dests,
      int tag, int source_tag, RouteTypeDP type, IRValue ir_value)
      : location(std::move(location)),
        sources(std::move(sources)),
        dests(std::move(dests)),
        tag(tag),
  ...
  friend std::ostream& operator<<(std::ostream& os, const RouteDP& route) {
    os << "loc: " << toString(route.location)
       ...
       << " _dest_order: " << route.dest_order << " _tag: " << route.tag
       << " _source_tag: " << route.source_tag
       << " _type: " << toString(route.type);
    return os;
  }

  MSGPACK_DEFINE(
      location, sources, source_graph, source_order, dests, dest_graph,
      dest_order, tag, source_tag, type, ir_value);
};
...
struct MPICall {
  ...
  MPICall(
      std::unordered_set<int> a_src, std::unordered_set<int> a_dest, int a_tag,
      bool a_blocking)
      : sources(std::move(a_src)),
        dests(std::move(a_dest)),
...
enum class CommProgress { NOT_STARTED, WAITING, RECEIVED, DONE };
...
MPI_Datatype getMPIDataType() {
  throw std::invalid_argument(
      "@CommCommon: Failed to get MPI data type. Unsupported data type.");
}
