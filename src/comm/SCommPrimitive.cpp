...
namespace rannc {
    ...
    torch::jit::IValue bcastPrimitive(const torch::jit::IValue& ivalue, const IRType& ir_type, int root, MPI_Comm communicator) {
        ...
        switch (ir_type.getScalarType()) {
            ...
            case IRScalarType::DEVICE: {
                ...
                mpi::checkMPIResult(MPI_Bcast(&dev_id, 1, getMPIDataType<int>(), root, communicator));

                torch::jit::IValue dev_iv;
                if (deviceFromInt(dev_id) == c10::DeviceType::CPU && torch::cuda::is_available()) {
                    // cuda is NOT available at source, but available here
                    dev_iv = torch::jit::IValue(c10::Device(c10::DeviceType::CUDA, getCurrentCudaDeviceId()));
                } else if (deviceFromInt(dev_id) == c10::DeviceType::CUDA && !torch::cuda::is_available()) {
            ...
            default:
                throw std::invalid_argument("bcastPrimitive receives a scalar, but the type is unsupported: " + toString(ir_type.getScalarType()));
        }
    };
    ...
    torch::jit::IValue bcastPrimitiveArray(const torch::jit::IValue& ivalue, const IRType& ir_type, int root, MPI_Comm communicator) {
        ...
        switch (ir_type.getListType()) {
            case IRListType::INT:
                return vectorToList(doBcastPrimitiveArray<int64_t>(ivalue, ir_type, root, communicator));
            case IRListType::FLOAT:
                return vectorToList(doBcastPrimitiveArray<double>(ivalue, ir_type, root, communicator));
            case IRListType::BOOL:
                return vectorToList(doBcastPrimitiveArray<bool>(ivalue, ir_type, root, communicator));
            default:
                throw std::invalid_argument("bcastPrimitiveArray receives a list, but the element type is unsupported: " + toString(ir_type.getListType()));
        }
    }
}
