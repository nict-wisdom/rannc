...
namespace rannc {
     ...
     */
    std::unordered_map<int, int64_t> getLocalSplitBatchSizes(const size_t split_batch_size, const std::unordered_set<int>& ranks,
                                                        int split_index) {
     ...
     *
     * @param batch_size The global batch size (the total batch size processed by all ranks and all splits).
    ...
    std::vector<int64_t> getSplitBatchSizes(int pipeline_num, int64_t batch_size) {
        ...
        const auto batch_sizes = getLocalSplitBatchSizes(batch_size, vectorToSet(dummy_ranks), 0);
    ...
    BatchSizeCalculator::BatchSizeCalculator(int pipeline_num, int64_t global_batch_size) :
    ...
    int64_t BatchSizeCalculator::getLocalSplitBatchSize(const std::unordered_set<int>& ranks, int my_rank, int split_index) const {
        assert(contains(ranks, my_rank));
        int64_t split_bs = getGlobalSplitBatchSize(split_index);

        std::unordered_map<int, int64_t> split_bs_all_ranks = getLocalSplitBatchSizes(split_bs, ranks, split_index);
        assert(contains(split_bs_all_ranks, my_rank));

        return split_bs_all_ranks.at(my_rank);
    }

    std::vector<int64_t> BatchSizeCalculator::getAllLocalSplitBatchSizes(const std::unordered_set<int>& ranks, int my_rank) const {
        assert(contains(ranks, my_rank));
        std::vector<int64_t> split_sizes;

        for (int split_index=0; split_index<split_batch_sizes_.size(); split_index++) {
            int64_t split_bs = getGlobalSplitBatchSize(split_index);
            std::unordered_map<int, int64_t> split_bs_all_ranks = getLocalSplitBatchSizes(split_bs, ranks, split_index);
            assert(contains(split_bs_all_ranks, my_rank));
    ...
    double BatchSizeCalculator::getDpRatio(const std::unordered_set<int>& ranks, int my_rank, int split_index) const {
        return getLocalSplitBatchSize(ranks, my_rank, split_index) / (double) getGlobalSplitBatchSize(split_index);
    }

    /**
     * Calculates the batch size for the given split index and scales the size of the head dimension.
     * Notice that:
     * - batch_size is the one that should be processed by all the ranks, but only for the given split index.
     *   This is not an effective batch size for a training iteration and can be calculated by
     *   std::vector<int64_t> getSplitBatchSizes(int pipeline_num, int64_t batch_size) .
     * - The head dimension may not equal the batch size.
     * Even for the same *batch_size*, the local batch size that should be processed on a certain rank can differ
     * if the split_index is different.
     *
     ...
     * @return
     */
    std::unordered_map<int, std::vector<int64_t>> BatchSizeCalculator::calcDistBatchDims(
            const std::vector<int64_t> &global_dim, const std::unordered_set<int> &ranks, int split_index) const {
        ...
        int64_t split_bs = getGlobalSplitBatchSize(split_index);
        std::unordered_map<int, int64_t> batch_sizes = getLocalSplitBatchSizes(split_bs, ranks, split_index);
    ...
    void BatchSizeCalculator::setPipeline(int pipeline_num, int64_t global_batch_size) {
