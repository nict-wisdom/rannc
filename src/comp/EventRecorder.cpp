...
std::string EventRecorder::dump() {
  ...
  const std::vector<std::vector<Event>> gathered_events =
      ocomm.allgather(vec_events, MPI_COMM_WORLD);
...
std::string getFuncKey(
  ...
  ss_key << prefix << "::" << func << "_id=" << id << "_split=" << split
         << "_grad=" << grad;
  return ss_key.str();
}
...
std::string getCommKey(
  ...
  std::stringstream ss_key;
  ss_key << prefix << "::" << direction << "_val=" << r.location.value_name
         << "_src=" << r.source_graph << "_dest=" << r.dest_graph
         << "_split=" << split;
  return ss_key.str();
}
...
std::string getCommKey(
  ...
  std::stringstream ss_key;
  ss_key << getCommKey(prefix, direction, r, split)
         << "_type=" << toString(type);
  return ss_key.str();
}
...
std::string getCopyKey(
  ...
  std::stringstream ss_key;
  ss_key << prefix << "::copy"
         << "_name=" << name << "_type=" << toString(type)
