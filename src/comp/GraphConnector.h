...
class GraphConnector {
 public:
  ...
  std::unordered_map<std::string, IValueMap> forward(
      ...
      bool grad_mode);
  std::unordered_map<std::string, IValueMap> backward(
      const std::string& id,
 ...
 private:
  ...
  // for debugging
  std::unordered_map<std::string, std::unordered_map<int, IValueMap>>
      last_cp_outputs_;
  ...
  std::unordered_map<int, std::unordered_map<std::string, IValueMap>>
      split_values_;
  int pipeline_num_;
  int max_fwd_delay_;
  int max_bwd_delay_;
  std::unordered_map<
      std::string,
      std::unordered_map<IValueLocation, size_t, IValueLocationHash>>
      fwd_inc_edges_count_;
  std::unordered_map<
      std::string,
      std::unordered_map<IValueLocation, size_t, IValueLocationHash>>
      bwd_inc_edges_count_;

  std::unordered_map<std::string, std::unordered_map<int, RngState>>
      rng_states_;
  std::unordered_map<int, bool> skip_fwd_split_;
  std::unordered_map<std::string, std::unordered_map<int, at::cuda::CUDAEvent>>
      copy_to_cpu_events_;
  std::unordered_map<std::string, std::unordered_map<int, at::cuda::CUDAEvent>>
      copy_to_gpu_events_;

  torch::jit::IValue distributeOutput(
      bool is_bwd, const RouteDP& r, int split_index, int flush_offset,
      const std::unordered_map<std::string, int>& graph_order);
  std::unordered_map<std::string, IValueMap> compute(
