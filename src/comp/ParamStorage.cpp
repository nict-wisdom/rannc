...
std::vector<long> getSortedParamIDs(
    const std::unordered_map<std::string, long>& param_map) {
...
GradConsolidation::GradConsolidation(
    ParamStorage* param_storage, std::vector<long> param_ids,
    bool use_amp_master_params, bool consolidate_master_params)
    : param_ids_(std::move(param_ids)),
      param_storage_(param_storage),
      use_amp_master_params_(use_amp_master_params),
      consolidate_master_params_(consolidate_master_params) {
  std::unordered_map<at::ScalarType, int64_t, EnumHash<at::ScalarType>>
      grad_elem_sum;
  std::unordered_map<
      at::ScalarType, std::unordered_map<long, int64_t>,
      EnumHash<at::ScalarType>>
      offsets;
  ...
  for (long pid : param_ids_) {
    ...
    assert(
        stype == at::ScalarType::Float || stype == at::ScalarType::Half ||
  ...
  for (long pid : param_ids_) {
    ...
    if (consolidate_master_params_ && stype == at::ScalarType::Half) {
      amp_master_grad_tensors_[pid] = consolidated_master_grads_[stype]
                                          .narrow(0, offset, p.numel())
                                          .view(p.sizes())
                                          .detach();
      continue;
    }

    grad_tensors_[pid] = consolidated_grads_[stype]
                             .narrow(0, offset, p.numel())
                             .view(p.sizes())
                             .detach();
    if (stype == at::ScalarType::Float && use_amp_master_params_ &&
        contains(consolidated_grads_, at::ScalarType::Float)) {
      assert(consolidated_amp_fp32_next_.defined());

      amp_grad_tensors_next_[pid] =
          consolidated_amp_fp32_next_.narrow(0, offset, p.numel())
              .view(p.sizes())
              .detach();
    } else {
...
void GradConsolidation::consolidate() {
  if (use_amp_master_params_ &&
    ...
    std::unordered_map<long, at::Tensor> tmp_grad_tensors =
        amp_grad_tensors_next_;
...
void ParamStorage::registerParam(
...
std::unordered_map<std::string, long> ParamStorage::getParamIDs(
...
at::Tensor ParamStorage::getParamTensor(
    const std::string& graph_id, const std::string& name) const {
...
IRType ParamStorage::getParamType(
    const std::string& graph_id, const std::string& name) {
...
void ParamStorage::allReduceParamGrads(const std::string& graph_id) {
  ...
  bool sync_allreduce =
      config::Config::get().getVal<bool>(config::SYNC_ALLREDUCE);
...
void ParamStorage::doScaleGrads(
...
void ParamStorage::scaleGrads(
    const std::string& graph_id, bool amp_master_grads) {
  doScaleGrads(graph_id, false, amp_master_grads);
}

void ParamStorage::unscaleGrads(
    const std::string& graph_id, bool amp_master_grads) {
...
std::tuple<int64_t, int64_t> ParamStorage::getLocalParamRange(
    long param_id) const {
  for (const auto& it : zero_grad_locators_) {
    for (const auto& param_it : getParamIDs(it.first, false)) {
      if (param_it.second == param_id) {
...
at::Tensor ParamStorage::gatherTensorZero(
...
void ParamStorage::registerAmpMasterParam(
    long model_param_id, long master_param_id, const at::Tensor& param_tensor) {
  amp_master_params_[model_param_id] = param_tensor;
  amp_param_id_map_[master_param_id] = model_param_id;
}

void ParamStorage::clipGradNorm(
...
double ParamStorage::calcGradGlobalL2Norm(
  ...
  std::unordered_map<std::string, long> params_on_rank =
      getParamIDs(graph_id, false);
  ...
  if (contains(unused_params_, graph_id)) {
    std::unordered_map<std::string, long>& params_not_on_rank =
        unused_params_.at(graph_id);
  ...
  ObjectComm& ocomm = ObjectComm::get();
  std::vector<std::unordered_map<long, float>> gathered_norms =
      ocomm.allgather(norms, MPI_COMM_WORLD);
...
void ParamStorage::deploy(
  ...
  std::unordered_map<
      ...
      param_rank_maps = getParamRanks(decomp);
  ...
  for (const auto& it : param_graph_set) {
    if (it.second.size() > 1) {
      std::stringstream ss;
      ss << "Parameter sharing among graphs on the same rank is not allowed: rank="
         << it.first.second << " name=" << it.first.first
         << " graphs=" << join_as_str(it.second);
  ...
  for (const auto& param_id : sorted_param_ids) {
    ...
    if (!contains(tag_rank_set_, comm_tag)) {
      if (contains(param_ranks, mpi::getRank())) {
        logger->trace(
            "Creating MPI communicator for params: tag={} ranks={}", comm_tag,
            join_as_str(param_ranks));

        MPI_Comm param_comm = scomm.getCommunicator(comm_tag, param_ranks);
        logger->trace(
            "Finished creating MPI communicator for params: tag={} ranks={}",
            comm_tag, join_as_str(param_ranks));
        MPI_Barrier(param_comm);

        logger->trace(
            "Creating communicator for params: tag={} ranks={}", comm_tag,
            join_as_str(param_ranks));
        ar.createCommunicator(comm_tag, param_ranks);
        logger->trace(
            "Finished creating communicator for params: tag={} ranks={}",
            comm_tag, join_as_str(param_ranks));
      }
    }
  ...
  if (enable_zero) {
    ...
    for (const auto& param_id : sorted_param_ids) {
      ...
      if (contains(param_ranks, mpi::getRank())) {
        at::Tensor& param_tensor = params_.at(param_id);
        zero_grad_locators_[graph_id]->registerGrad(
            param_id, param_tensor, param_ranks);
        logger->trace(
            "Registered param for zero: pid={} ranks={}", param_id,
            join_as_str(param_ranks));
      }
    }
    ...
    for (const auto& it : graph_grouped_params) {
      ...
      if (contains(ranks, mpi::getRank())) {
        ...
        grad_cons_[graph_id][it.first] = std::make_shared<GradConsolidation>(
            this, it.second, use_amp_master_params_.at(graph_id),
            allreduce_amp_master_params_);
      }
    }
  }

  logger->trace(
      "ParamStorage::deploy deployed all params. graph_id={}", decomp.id);
}

void ParamStorage::syncParamOnInit(
    long param_id, const std::unordered_set<int>& ranks) {
  at::Tensor& param_tensor = params_.at(param_id);
...
at::Tensor ParamStorage::doSyncParam(long param_id, bool grad) {
  ...
  if (contains(param_ranks, mpi::getRank())) {
    ...
    buf = createTensorFromIRType(
        sync_ir_type, c10::Device(c10::DeviceType::CUDA));
  }
