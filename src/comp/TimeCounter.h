...
namespace rannc {
    ...
    private:
        ...
        std::unordered_map<std::string, std::chrono::system_clock::duration> duration_;
    ...
    long long TimeCounter::get(const std::string& task) const {
        if (enabled_) {
            ...
            long long average = std::chrono::duration_cast<U>(duration_.at(task)).count() / count_.at(task);
            return average;
        }
        return -1;
    }
    ...
    std::string TimeCounter::summary() const {
        if (enabled_) {
            ...
            for (const auto &task: getTasks()) {
                ...
                ss << "Task " << task << ": average time=" << get<U>(task) << " count=" << count << std::endl;
            }
            return ss.str();
        }
    ...
    T measureTime(F f, int iter, int min_iter, TimeCounter &counter, const std::string &id) {
        if (iter >= min_iter) {
            counter.start(id);
        }
        T ret = f();
    ...
    template <typename F>
    void measureTime(F f, int iter, int min_iter, TimeCounter &counter, const std::string &id) {
        if (iter >= min_iter) {
            counter.start(id);
        }
        f();
