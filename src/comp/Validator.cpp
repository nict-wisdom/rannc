...
namespace rannc {
    ...
    bool almostEqual(const at::Tensor& t1, const at::Tensor& t2, double tolerance, double tolerance_ratio) {
    ...
    std::vector<torch::jit::IValue> orderOutputs(const IValueMap& outputs, const std::shared_ptr<IRGraph>& graph) {
    ...
    std::unordered_map<std::string, at::Tensor> graphParamTensors(const std::shared_ptr<IRGraph> &graph,
    ...
    IValueMap compute(const IValueMap& inputs,
                 const std::shared_ptr<IRGraph>& graph,
                 const std::unordered_map<std::string, torch::jit::IValue>& params,
                 const IValueMap& const_vals,
                 const FunctionStorage& functions) {

        std::unordered_map<std::string, at::Tensor> graph_params = graphParamTensors(graph, params);
        ...
        for (const auto& it: graph_params) {
            params_gpu[it.first] = it.second.to(torch::Device(torch::kCUDA), false, true).detach();
        }

        TorchDriver driver;
        driver.createModule(graph->getName(), graph, const_vals,
                            functions, params_gpu);
    ...
    bool Validator::validate(const std::shared_ptr<torch::jit::Graph>& graph,
        ...
        auto func = std::make_shared<torch::jit::GraphFunction>("forward", no_dropout_graph, nullptr);
        ...
        try {
            ...
            for (size_t i = input_ivals.size(); i < func->num_inputs(); i++) {
                ...
                stack.push_back(toCUDAIfAvailable(param_inputs.at(in_val->debugName()), true));
            }

            func->run(stack);
        } catch (std::exception& e) {
            logger->error("Failed to run traced model. "
        ...
        for (const auto& sg_name: deployment.fwd_graph_order) {
            ...
            sg_output_vals[sg_name] = compute(sg_inputs, sg, param_inputs, const_vals_no_do, functions);
        ...
        for (size_t i=0; i<stack.size(); i++) {
            ...
            if (!sameShape(expected, actual)) {
                logger->error("Resulting shapes do not match. expected={} actual={}",
            ...
            if (!almostEqual(expected, actual, 0.001, 0.01)) {
                logger->error("Results do not match. expected={} actual={}",
                             tensorToString(expected),
                             tensorToString(actual));
                return false;
            }
        }
        return true;
