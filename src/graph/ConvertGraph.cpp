...
namespace {
    ...
    bool containsIRValue(const std::vector<rannc::IRValue>& value_set, const std::string& name) {
...
namespace rannc {
            ...
            std::unordered_map<std::string, torch::jit::Value *> & regValues)
    {
        ...
        torch::jit::Value * fn_constant = node->s_(c10::attr::name, attr_name)
                ->output()
                ->setType(c10::FunctionType::create(callee));
        regValues[out_name] = fn_constant
                ->setDebugName(out_name);
    ...
    void ConvertGraph::doToTorch(
        ...
        const FunctionStorage::FunctionTable &functions = function_storage.getFunctions();
...
//            logger()->info("Adding node. name={}", irNode.getName());
            ...
            if ( irGraph->isFunctionNode(irNode) ) {
                ...
                auto n = graph->insertNode(graph->create(c10::prim::CallFunction, inputs));
...
//                logger()->info("  Inserting constant. name={} outputs={}", irNode.getName(),
...
//                logger()->info("  Inserting ListConstruct. name={} elem_num={}", irNode.getName(),
...
//                    logger()->info("  adding list input. name={}", v->debugName());
...
//                    logger()->info("  adding list output. name={} irtype={}", v->debugName(), toString(val.getType()));
...
//                    logger()->info("  added list output. name={} type={}", v->uniqueName(),
//                                 typeKindToString(fromIRType(val.getType())->kind()));
                }
            } else {
//                logger()->info("  Inserting op. name={}", irNode.getName());

                auto n = graph->create(torch::jit::Symbol::fromQualString(irNode.getName()),
                ...
                for (const std::string &inName: irNode.getInputNames()) {
                    ...
                    if (containsIRValue(no_param_inputs, inName)
                        ...
                        // AND reuse of inputs for checkpointing may be corrupted due to in-place operations
                        torch::jit::Value* none = graph->insertNode(graph->createNone())->output();
                        auto clone_node = graph->create(torch::jit::Symbol::fromQualString("aten::clone"), 1);
...
//                        logger()->info("    adding node input. name={}", irNode.getName());
                        n->addInput(in_val);
                    }
                }
                graph->appendNode(n);
...
//                    logger()->info("    adding node output. name={}", v->uniqueName());

                    if (!contains(values, name)) {
                        std::stringstream ss;
                        ss << "Failed to find output. node=" << irNode.getName()
                            << " out=" << name
                            << " values=" << join_as_str(keys(values));
...
//            *irGraph << std::endl;
        ...
        merged_input->setType(static_cast<at::TypePtr>(at::TupleType::create(elements)));
        ...
        doToTorch(graph, regValues, irGraph, constants,
                  functions, no_param_inputs);
    ...
    torch::jit::Value *addInput(std::shared_ptr<torch::jit::Graph> &graph,
        ...
        switch (base_type) {
            ...
            case IRBaseType::TENSOR: {
                if (ir_type.getTensorElemType() == IRTensorElemType::UNDEF) {
                    ...
                    at::ScalarType tensor_elem_type = fromIRTensorElemTypeToScalarType(ir_type.getTensorElemType());
                    auto &tensor_dim = ir_type.getTensorDim();
                    return torch::jit::TensorType::createContiguous(tensor_elem_type, at::kCPU, tensor_dim);
                }
            }
...
//                        spdlog::info("{} is reachable from {}", bg[tgt].name, bg[in].name);
        ...
        return std::make_shared<IRGraph>(g->getName(), nodes, values,
        ...
        for (const auto &in_name: g->getInputNames()) {
            ...
            if (type.getBaseType() == IRBaseType::TENSOR) {
                ...
                if (bsize < 1) {
                    ...
                    logger()->info("Failed to guess a batch size. Detected different batch sizes: {} and {}",
                                   bsize, a_bsize);
                    return g;
                }
            }
    ...
    std::shared_ptr<IRGraph> setValueTypes(const std::shared_ptr<IRGraph> &g,
        ...
        for (auto &it: values) {
            ...
            if (!contains(value_types, it.first)) {
                throw std::invalid_argument("No type information in profiling results: " + it.first);
            }
...
//                logger()->info("   node_out: {} type={} ir_type={} torch_type={}", node_out->debugName(), typeKindToString(node_out->type()->kind()),
//                               toString(toIRValue(node_out)), node_out->type()->annotation_str());
//            }
            for (torch::jit::Value *node_out: node->outputs()) {
            ...
            std::transform(node->inputs().begin(), node->inputs().end(), inputNames.begin(),
                           [](torch::jit::Value *n) { return n->debugName(); });
            std::vector<std::string> outputNames(node->outputs().size());
            std::transform(node->outputs().begin(), node->outputs().end(), outputNames.begin(),
                           [](torch::jit::Value *n) { return n->debugName(); });
        ...
        std::transform(graph->inputs().begin(), graph->inputs().end(), graphInputNames.begin(),
                       [](torch::jit::Value *n) { return n->debugName(); });
        std::vector<std::string> graphOutputNames(graph->outputs().size());
        std::transform(graph->outputs().begin(), graph->outputs().end(), graphOutputNames.begin(),
                       [](torch::jit::Value *n) { return n->debugName(); });

        auto ir_graph = std::make_shared<IRGraph>(name, nodes, values, graphInputNames, graphOutputNames);
