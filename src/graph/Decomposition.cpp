...
namespace rannc {
    ...
    typedef boost::filtered_graph<BGraph, std::function<bool(BGraph::edge_descriptor)>,
            std::function<bool(BGraph::vertex_descriptor)>> ComponentGraph;
...
//            logger->trace("   Value name={} id={} param={} input={} output={}", b_graph[v].name, b_graph[v].id,
//                          b_graph[v].is_param, b_graph[v].is_input, b_graph[v].is_output);
        }
...
//            logger->trace("Node name={} id={}", b_graph[v].name, b_graph[v].id);

            for (const auto &input_name: in_node.getInputNames()) {
                Vertex in_v = values[input_name];
                boost::add_edge(in_v, v, b_graph);
//                std::cout << "  Created edge: " << (*bGraph)[in_v].name << " -> " << (*bGraph)[v].name << std::endl;
            }
...
//                logger->trace("   Value name={} id={} param={} input={} output={}", b_graph[out_v].name,
//                              b_graph[out_v].id,
//                              b_graph[out_v].is_param, b_graph[out_v].is_input, b_graph[out_v].is_output);

                //                std::cout << "  Created edge: " << (*bGraph)[v].name << " -> " << (*bGraph)[out_v].name << std::endl;
            }
        }
    ...
    bool containsVertex(const BGraph &g, const std::vector<Vertex> &vertices, const Vertex tgt) {
    ...
    BGraph createSubgraph(const BGraph &g, const std::vector<Vertex> &sub_vertices) {
    ...
    int getRank(std::unordered_map<int, BGraph> &subgraphs, const std::string &id) {
        for (auto &sg: subgraphs) {
            if (containsVertexById(sg.second, id)) return sg.first;
        }
    ...
    bool containsConnection(const std::vector<GraphConnection> &connections, const GraphConnection &con) {
        bool found = false;
    ...
    bool hasSameValueConnection(const std::vector<GraphConnection> &connections, const std::string &value_name,
                                const std::string &dest_sg) {
        for (const auto &c: connections) {
...
//                    spdlog::info("Checking graph edge: {}@rank{}->{}@rank{}", g[src].name, src_rank, g[tgt].name, tgt_rank);
                    const auto &src_graph_id = subgraph_id_map[src_rank];
                    const auto &tgt_graph_id = subgraph_id_map[tgt_rank];

                    GraphConnection con{g[src].name, src_graph_id, tgt_graph_id, g[src].id, g[tgt].id};
                    if (g[src].is_param) {
                        if (!containsConnection(param_connections, con)) {
                            param_connections.emplace_back(con);
//                            spdlog::info("Found shared param connection: {} {}->{}", g[src].name, src_graph_id, tgt_graph_id);
                        }
                    } else {
                        if (!containsConnection(connections, con)) {
                            connections.emplace_back(con);
//                            spdlog::info("Created graph connection: {} {}->{}", g[src].name, src_graph_id, tgt_graph_id);
                        }
                    }
                }
            }
...
//
//                        spdlog::info("Created graph connection: {} MASTER->{}", vi.name, sg_id);
                    }
                }
...
//                    spdlog::info("Created graph connection: {} {}->MASTER", vi.name, sg_id);
    ...
    std::vector<IRValue> searchGraphValuesByName(const std::vector<std::shared_ptr<IRGraph>> &ir_graphs,
                                                 const std::string &val_name) {
        std::vector<IRValue> found;
    ...
    RGraph toRGraph(const std::unordered_map<std::string, std::shared_ptr<IRGraph>>& subgraphs,
    ...
    std::vector<std::string> sortGraphs(const std::unordered_map<std::string, std::shared_ptr<IRGraph>>& subgraphs,
            const std::vector<RouteDP> &routes) {
    ...
    Deployment createDeployment(const PartitionDP &partition,
        ...
        for (const auto &con: partition.connections) {
            ...
            logger->trace("Created route for forwarding: {} (batch={} loss={} con={})", toString(fwd_route),
                          val.isBatch(), val.isLoss(), toString(con));
        ...
        deployment.pipeline_num = config::Config::get().getVal<int>(config::PIPELINE_NUM);
    ...
    template<typename F>
    std::unordered_map<std::string, std::unordered_map<std::string, std::unordered_set<int>>> getGraphValueRanks(const Deployment &deployment, F f) {
        std::unordered_map<std::string, std::unordered_map<std::string, std::unordered_set<int>>> results;
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, std::unordered_set<int>>> getParamRanks(const Deployment &deployment) {
        return getGraphValueRanks(deployment, graphParamValues);
    }

    std::unordered_map<std::string, std::unordered_map<std::string, std::unordered_set<int>>> getConstantRanks(const Deployment &deployment) {
        return getGraphValueRanks(deployment, graphConstantValues);
    }

    std::vector<RouteDP> filterRoutes(const std::vector<RouteDP> &routes, int rank,
                                      std::function<std::vector<int>(const RouteDP &r)> &f) {
        std::vector<RouteDP> results;
        std::copy_if(routes.begin(), routes.end(), std::back_inserter(results),
                     [rank, &f](const RouteDP &r) {
                         return contains(f(r), rank);
                     });
        return results;
    }

    std::vector<RouteDP> filterRecvRoutes(const std::vector<RouteDP> &routes, int rank) {
        std::function<std::vector<int>(const RouteDP &r)> f = [](const RouteDP &r) { return r.dests; };
        return filterRoutes(routes, rank, f);
    }

    std::vector<RouteDP> filterSendRoutes(const std::vector<RouteDP> &routes, int rank) {
        std::function<std::vector<int>(const RouteDP &r)> f = [](const RouteDP &r) { return r.sources; };
        return filterRoutes(routes, rank, f);
    }

    std::vector<RouteDP> filterRoutesByGraph(const std::vector<RouteDP> &routes,
    ...
    std::vector<RouteDP> filterRoutesByGraphInputs(const std::vector<RouteDP> &routes,
                                                   const std::shared_ptr<IRGraph> &graph) {
        return filterRoutesByGraph(routes, graph->getInputNames());
    }

    std::vector<RouteDP> filterRoutesByGraphOutputs(const std::vector<RouteDP> &routes,
                                                    const std::shared_ptr<IRGraph> &graph) {
    ...
    std::unordered_map<std::string, GraphRoutes> getRoutesByGraph(
        ...
        for (const auto &subgraph: subgraphs) {
            ...
            graph_routes.fwd_recv_routes = filterRoutesByGraphInputs(filterRecvRoutes(deployment.fwd_routes, rank), subgraph);
            graph_routes.fwd_send_routes = filterRoutesByGraphOutputs(filterSendRoutes(deployment.fwd_routes, rank), subgraph);
            graph_routes.bwd_recv_routes = filterRoutesByGraphOutputs(filterRecvRoutes(deployment.bwd_routes, rank), subgraph);
            graph_routes.bwd_send_routes = filterRoutesByGraphInputs(filterSendRoutes(deployment.bwd_routes, rank), subgraph);
    ...
    std::vector<BGraph> getConnectedComponents(const BGraph &g) {
        std::shared_ptr<std::vector<unsigned long>> mapping = std::make_shared<std::vector<unsigned long>>(
                num_vertices(g));
        ...
        for (size_t i = 0; i < num; i++) {
            ComponentGraph comp = {g,
                                   [mapping, i, &g](BGraph::edge_descriptor e) {
                                       return mapping->at(source(e, g)) == i || mapping->at(target(e, g)) == i;
                                   },
            ...
            comp_g[boost::graph_bundle].id = createComponentGraphId(g[boost::graph_bundle].id, i);
    ...
    std::string searchGraphWithValue(const std::unordered_map<std::string, std::shared_ptr<IRGraph>> &ir_graphs,
            const std::string &val_name) {
    ...
    Partition createPartition(const BGraph &g) {
        ...
        std::unordered_map<std::string, std::unordered_map<std::string, std::shared_ptr<IRGraph>>> comp_graph_groups;
        ...
        for (const auto &it: b_decomp.graphs) {
            ...
            if (dump_graph) {
                ...
                boost::write_graphviz(file1, subg, vertex_rank_label_writer<BGraph>(subg));
            }
        ...
        // fix connections
//        spdlog::info("b_decomp.connections={}", b_decomp.connections.size());
...
//            spdlog::info("  con={}", toString(con));
            ...
            GraphConnection comp_con = {con.value, comp_src, comp_dest, con.id, con.targetId};
        ...
        logger->trace("Cut size: {} (cut count={})", cut_size, b_decomp.connections.size());

        auto ret = Partition{g[boost::graph_bundle].id, fromBGL(g), comp_graphs, comp_connection, b_decomp.order};
    ...
    std::unordered_map<int, IRType> getDistTensorType(const IRType& type, const std::unordered_set<int>& ranks,
        ...
        for (int r: ranks) {
            const auto dp_type = IRType::createTensorType(type.getTensorElemType(), dp_dim.at(r), type.requiresGrad());
            ret[r] = dp_type;
        }
        return ret;
    }

    IRType getDistType(const IRType& type, const std::unordered_set<int>& ranks, int64_t batch_size, int rank) {
        auto base_type = type.getBaseType();
        switch (base_type) {
            case IRBaseType::SCALAR: return type;
            ...
            case IRBaseType::LIST: {
                ...
                if (list_type == IRListType::TENSOR) {
                    ...
                    for (size_t i=0; i<type.getListSize(); i++) {
                        const auto dist_types = getDistTensorType(type.getCompoundTypes().at(i), ranks, batch_size);
    ...
    std::shared_ptr<IRGraph> scaleGraph(const std::shared_ptr<IRGraph>& graph, const std::string& name,
            int num, int index, int64_t batch_size) {
        ...
        return std::make_shared<IRGraph>(name, graph->getNodes(), values,
                                                 graph->getInputNames(), graph->getOutputNames());
    }
    ...
    std::vector<std::shared_ptr<IRGraph>> replicateGraph(const std::shared_ptr<IRGraph>& graph, int num, int64_t batch_size) {
    ...
    PartitionDP replicate(const PartitionDP &partition, const std::unordered_map<std::string, int>& repl_nums,
        ...
        for (const auto& it: repl_nums) {
            ...
            if (num > 1 && !g->isReplicable()) {
                logger->info("Graph {} is not replicable. Replication num {} is ignored.", it.first, it.second);
                num = 1;
            }
    ...
    bool isAllocFeasible(const std::unordered_map<int, size_t>& alloc_mem, size_t dev_mem) {
    ...
    bool hasReplica(const std::string& sg_name, int dev,
            const std::unordered_map<std::string, std::unordered_set<std::string>>& replications,
    ...
    std::unordered_map<std::string, int> doSearchAllocation(
        ...
        std::sort(alloc_states.begin(), alloc_states.end(), [](const AllocState& s1, const AllocState& s2) {
            return s1.getMaxAllocMem() < s2.getMaxAllocMem();
        });

        for (const auto& st: alloc_states) {
            const auto ret_alloc = doSearchAllocation(sg_names, replications, st.alloc, st.alloc_mem, graph_sizes, devices, dev_mem);
            if (!ret_alloc.empty()) {
                return ret_alloc;
            }
        }
...
//        }
//
//        return doSearchAllocation(sg_names, partition.replications, alloc, alloc_mem, graph_sizes, devices, dev_mem);
    }
    ...
    std::unordered_map<std::string, std::unordered_set<int>> searchAllocationSimple(
        ...
        std::sort(sorted_gid.begin(), sorted_gid.end(), [&partition](const std::string& g1, const std::string& g2) {
    ...
    std::unordered_map<std::string, std::unordered_set<int>> searchAllocationFitToDevice(
            const PartitionDP& partition) {
    ...
    // If a value is NOT a batch, replicates the value on the ranks of following ops.
...
//                    spdlog::info("Value {} {} is batch. ranks: {}", g[v].name, g[v].id,
...
//                spdlog::info("Value {} {} changed ranks: {} to {}", g[v].name, g[v].id,
//                        join_as_str(g[v].ranks),
//                             join_as_str(fixed_ranks));
                ...
                if (g[v].node.isBatch()) {
//                    spdlog::info("Node {} {} is batch. ranks: {}", g[v].name, g[v].id,
//                                 join_as_str(g[v].ranks));
                    continue;
                }
...
//                             g[v].name, g[v].id, join_as_str(output_names), join_as_str(g[v].ranks),
//                             join_as_str(fixed_ranks));
...
//            spdlog::info("@setRanksOnGraph {} ranks={}", g[v].name, join_as_str(g[v].ranks));
        }
    }
    ...
    BGraph copyGraphWithBatch(const BGraph& g) {
        ...
        typedef typename boost::graph_traits<BGraph>::out_edge_iterator out_edge_iterator;
        typedef typename boost::adjacency_iterator_generator<BGraph, Vertex, out_edge_iterator>::type adjacency_iterator;

        for (const auto& src: all_nodes_topo<Vertex, BGraph>(g)) {
            std::pair<adjacency_iterator, adjacency_iterator> iter_pair = boost::adjacent_vertices(src, g);
            for (auto first = iter_pair.first, last = iter_pair.second; first != last; ++first) {
                Vertex tgt = *first;
                if (contains(desc_map, g[src].id) && contains(desc_map, g[tgt].id)) {
                    boost::add_edge(desc_map.at(g[src].id), desc_map.at(g[tgt].id), copy_graph);
                }
            }
        }
        ...
        for (const auto& it: deployment.subgraphs) {
            ...
            for (const auto& in_name: ir_graph->getInputNames()) {
                ...
                for (const auto& r: fwd_routes) {
                    if (graph_name == r.dest_graph && r.ir_value.getName() == in_val.getName()) {
                        used_fwd = true;
                    }
                }
                if (!used_fwd) {
                ...
                if (passedForBackward(in_val.getType())) {
                    ...
                    for (const auto &r: bwd_routes) {
                        if (graph_name == r.source_graph && r.ir_value.getName() == in_val.getName()) {
                            used_bwd = true;
                        }
                    }
                    if (!used_bwd) {
            ...
            for (const auto& out_name: ir_graph->getOutputNames()) {
                ...
                bool used_fwd = false;
                for (const auto& r: fwd_routes) {
                    if (graph_name == r.source_graph && r.ir_value.getName() == out_val.getName()) {
                        used_fwd = true;
                    }
                }
                if (!used_fwd) {
                ...
                if (passedForBackward(out_val.getType())) {
                    bool used_bwd = false;
                    for (const auto &r: bwd_routes) {
                        if (graph_name == r.dest_graph && r.ir_value.getName() == out_val.getName()) {
                            used_bwd = true;
                        }
                    }
                    if (!used_bwd) {
