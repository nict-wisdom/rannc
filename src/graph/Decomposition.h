...
namespace rannc {
    ...
    typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS, VertexInfo, EdgeInfo, GraphInfo> BGraph;
    typedef boost::graph_traits<BGraph>::vertex_descriptor Vertex;
    typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::undirectedS, VertexInfo, EdgeInfo, GraphInfo> UndirectedBGraph;
    ...
    struct GraphConnection {
        ...
        friend std::ostream &operator<<(std::ostream &os, const GraphConnection &connection) {
            os << "value: " << connection.value << " src: " << connection.src << " dest: "
               << connection.dest << " id: " << connection.id << " targetId: " << connection.targetId;
            return os;
        }
    };
    ...
    struct GraphConnectionDP {
        ...
        friend std::ostream &operator<<(std::ostream &os, const GraphConnectionDP &connection) {
    ...
    typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS, RVertexInfo> RGraph;
    ...
    struct GraphRoutes {
        ...
        MSGPACK_DEFINE(fwd_send_routes, fwd_recv_routes, bwd_send_routes, bwd_recv_routes);
    };
    ...
    struct Deployment {
        ...
        friend std::ostream &operator<<(std::ostream &os, const Deployment &deployment);

        MSGPACK_DEFINE(id, graph, subgraphs, allocation, fwd_routes, fwd_in_routes, fwd_out_routes, fwd_graph_order,
                bwd_routes, bwd_in_routes, bwd_out_routes, bwd_graph_order, pipeline_num, checkpointing);
    };
    ...
    std::vector<Vertex> all_nodes_topo(const Graph &g) {
        ...
        boost::for_each(rev_topo_vertices | boost::adaptors::reversed, [&sorted](Vertex v) {
            sorted.push_back(v);
        });
        return sorted;
    }
    ...
    template<typename Vertex, typename Graph>
    std::vector<Vertex> graph_input_values(const Graph &g, std::function<bool(Vertex)> condition) {
        std::vector<Vertex> input_nodes = graph_input_nodes<Vertex, Graph>(g);
        std::vector<Vertex> val_inputs;
        std::copy_if(input_nodes.begin(), input_nodes.end(), std::back_inserter(val_inputs), condition);
        return val_inputs;
    }

    template<typename Vertex, typename Graph>
    std::vector<Vertex> graph_param_input_nodes(const Graph &g) {
        return graph_input_values<Vertex, Graph>(g, [&g](Vertex v) { return g[v].type == VALUE && g[v].is_param; });
    }
    ...
    std::vector<Vertex> target_nodes(const Vertex &v, const Graph &g) {
        ...
        typedef typename boost::adjacency_iterator_generator<Graph, Vertex, out_edge_iterator>::type adjacency_iterator;

        std::pair<adjacency_iterator, adjacency_iterator> iter_pair = boost::adjacent_vertices(v, g);
    ...
    void print_is_reachable(Vertex v_start, Vertex v_goal, const Graph &g) {
        ...
        if (boost::is_reachable(v_start, v_goal, g, color.data())) {
            std::cout << "REACHABLE: " << g[v_start].name << " -> " << g[v_goal].name << std::endl;
        } else {
            std::cout << "UNREACHABLE: " << g[v_start].name << " -> " << g[v_goal].name << std::endl;
        }
    }

    template<typename Vertex, typename Graph>
    Vertex addSubgraphVertex(const Graph &parent, const Vertex &pv, Graph &subgraph, std::unordered_map<Vertex, Vertex> &vmap) {
        ...
        void write(std::ostream& out, const Vertex& vertex) const
        {
            ...
            if (graph_[vertex].ranks.empty()) {
                ...
                switch (*graph_[vertex].ranks.begin() % 6) {
                    ...
                    case 1: color = "antiquewhite"; break;
                    case 2: color = "azure"; break;
                    case 3: color = "gold1"; break;
                    case 4: color = "lightpink"; break;
                    case 5: color = "olivedrab1"; break;
                }
            }
            ...
            if (graph_[vertex].type == NODE) {
                ...
                out << "[label=\"" << label_ss.str()
                    << "\", shape=\"" << shape
                    << "\", id=\"" << graph_[vertex].id
                    << "\", rank=\"" << join_as_str(graph_[vertex].ranks)
                    << "\", style=\"" << "filled"
                    << "\", fillcolor=\"" << color
                    << "\"]";
            } else {
    ...
    std::shared_ptr<IRGraph> fromBGL(const Graph& b_graph) {
        ...
        return std::make_shared<IRGraph>(b_graph[boost::graph_bundle].id, nodes, values, input_names, output_names);
    }
...
//    void fixConstantRank(BGraph &g);
    ...
    typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS, std::shared_ptr<IRGraph>> HGraph;
