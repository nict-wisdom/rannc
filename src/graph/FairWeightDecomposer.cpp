...
size_t calcValueSize(const IRValue& value, int replica_num) {
  ...
  if (value.isParam()) {
    // under testing, param + gradient + amp master param + buffer*2 (1 + 1 + 2
    // + 2*2)
    //            return val_size * 9;
    return val_size;
...
void chooseRanksOnNonCriticalPaths(BGraph& g) {
  ...
  for (auto& v : rev_topo_vertices) {
    if (g[v].ranks.empty()) {
      ...
      //                spdlog::info("{} has no rank. set to {}", g[v].name,
...
void splitByCriticalPath(BGraph& g, int n_partition) {
  ...
  std::vector<boost::graph_traits<BGraph>::vertices_size_type> distances(
  ...
  auto parents_pmap = boost::make_iterator_property_map(
      parents.begin(), get(boost::vertex_index, g));
  ...
  for (auto& in_v : input_nodes) {
    ...
    auto visitor =
        boost::visitor(boost::make_bfs_visitor(std::make_pair(v1, v2)));
    boost::breadth_first_search(g, in_v, visitor);

    for (const auto& out_v : output_nodes) {
      spdlog::info(
          "path length from {} to {}: {}", g[in_v].name, g[out_v].name,
          dist_pmap[out_v]);
...
Deployment FairWeightDecomposer::decompose(
    const std::shared_ptr<IRGraph>& irGraph) {
  int conf_n_partition =
      config::Config::get().getVal<int>(config::PARTITION_NUM);
  ...
  if (n_partition <= 0) {
    ...
    logger->info(
        "The partition num was automatically set to {}. To manually set this value, "
        "add 'partition_num' to ~/.pyrannc/rannc_conf.toml",
        n_partition);
  }
  ...
  for (const auto& it : partition.subgraphs) {
    ...
    logger->trace(
        " AFTER createPartition: Subgraph {} size={} param_size={}", name,
        sg->getSizeInByte(), sg->getParamSizeInByte());
  }

  // Use IRGraph in the argument because createPartition loses the order of
  // inputs/outputs
  partition.graph = irGraph;
  logger->trace(
      "FairWeightDecomposer::decompose created partition. id={}", partition.id);

  logger->trace(
      "FairWeightDecomposer::decompose creating replications. id={} repl_num={}",
      partition.id, replica_num);
  ...
  PartitionDP partitionDp =
      replicate(partition, repl_nums, conf_n_pipeline, batch_size_);
  logger->trace(
      "FairWeightDecomposer::decompose created PartitionDP. id={}",
      partitionDp.id);

  //        logger->info("Created subgraphs: partitions={} replications={}",
  //        n_partition, replica_num); for (const auto& it:
  //        partitionDp.subgraphs) {
  //            const auto& name = it.first;
  //            const auto& sg = it.second;
  ////            logger->info(" AFTER replicate: Subgraph {} size={}
  /// opt_param_size={} sum={}", name, sg->getSizeInByte(),
  /// sg->getParamSizeInByte()*6, /                         sg->getSizeInByte()
  /// + sg->getParamSizeInByte()*6);
  //        }

  //        const auto alloc_mem = sg_prof_->profile(partitionDp.subgraphs);
  //        for (const auto& it: alloc_mem) {
  //            const auto& prof = it.second;
  //            logger->info("{} fwd_time={} bwd_time={} mem={}", it.first,
  //            prof.fwd_time, prof.bwd_time, prof.max_allocated_mem);
  //        }

  logger->info(
      "Assigning {} subgraphs to {} device(s) ... (mem: {} per device) pipeline={}",
      partitionDp.subgraphs.size(), mpi::getSize(), dev_mem_, conf_n_pipeline);

  std::unordered_map<std::string, std::unordered_set<int>> alloc =
      searchAllocation(partitionDp, mpi::getSize(), dev_mem_ * 2);
  if (alloc.empty()) {
  ...
  deployment.checkpointing =
      config::Config::get().getVal<bool>(config::CHECKPOINTING);
