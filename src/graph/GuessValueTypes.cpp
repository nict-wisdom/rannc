...
namespace rannc {
    ...
    enum class ValueType {
        BATCH, PARAM, CONST, LOSS, UNKNOWN
    };


    std::string toString(ValueType type) {
        switch (type) {
            case ValueType::BATCH: return "BATCH";
            case ValueType::PARAM: return "PARAM";
            case ValueType::CONST: return "CONST";
            case ValueType::LOSS: return "LOSS";
            case ValueType::UNKNOWN: return "UNKNOWN";
        }
        throw std::invalid_argument("Failed to convert ValueType to string.");
    }

    std::vector<ValueType> toValueTypes(const std::vector<Vertex>& vertices, std::unordered_map<std::string, ValueType>& types,
                                        const BGraph& bg) {
    ...
    std::vector<IRType> toIRTypes(const std::vector<Vertex>& vertices, const BGraph& bg) {
    ...
    bool matchLossRule(const std::vector<ValueType>& src_types, const std::vector<IRType>& tgt_ir_types) {
    ...
    bool matchBatchRule(const std::vector<ValueType>& src_types, const std::vector<IRType>& tgt_ir_types) {
        // count batch
        int batch_count = 0;
        for (const auto& t: src_types) {
            if (t == ValueType::BATCH) {
...
//                    spdlog::info("match {} CONST", bg[v].name);
        ...
        const auto typed_graph = std::make_shared<IRGraph>(g->getName(), nodes, values,
...
//        boost::write_graphviz(file, typed_bg, vertex_rank_label_writer<BGraph>(typed_bg));
