...
namespace rannc {
    ...
    std::vector<MLNode> adj_ml_nodes(const MLNode& n, const MLGraph& graph, const std::vector<std::string>& ids) {
    ...
    std::vector<MLEdge> ml_edge(const MLNode& src, const MLNode& tgt, const std::vector<MLEdge>& edges) {
    ...
    std::unordered_set<std::string> getRequiredInputs(const std::shared_ptr<IRGraph>& g1,
                                                        const std::shared_ptr<IRGraph>& g2,
    ...
    std::unordered_set<std::string> getRequiredInputs(const MLNode& n1, const MLNode& n2,
    ...
    std::unordered_set<std::string> getPreservedOutputs(const std::string& src_node_id,
            const std::string& moved_graph_id, const std::vector<MLNode>& all_nodes) {
    ...
    std::vector<IRValue> getCutValues(const std::shared_ptr<IRGraph>& g1, const std::shared_ptr<IRGraph>& g2) {
...
//        spdlog::info("Cuts between {} and {}: {}", g1->getName(), g2->getName(), join_as_str(cut_names));
    ...
    bool outputAvailable(const IRNode& n, const std::unordered_set<std::string>& avail_vals) {
    ...
    std::shared_ptr<IRGraph> merge(const std::shared_ptr<IRGraph>& g1, const std::shared_ptr<IRGraph>& g2,
        ...
        for (const auto& i2: (getNonParamInputNames(g2))) {
            if (!contains(g1->getOutputNames(), i2) && !contains(non_param_inputs, i2)) {
        ...
        std::unordered_set<std::string> node_out_names = vectorToSet(non_param_inputs);
        ...
        return std::make_shared<IRGraph>(generateName("ML_"), nodes, merged_values, all_inputs, outputs);
    }
        ...
        for (const auto& node: g->getNodes()) {
            for (const auto& out: node.getOutputNames()) {
                if (!contains(values, out)) {
                    spdlog::error("{} does not have output of {}: {}",
                            g->getName(), node.getName(), out);
                    return false;
                }
                if (contains(g->getInputNames(), out)) {
                    spdlog::error("{} has {} as input, but the value is an output of {}",
    ...
    MLNode merge(const MLNode& n1, const MLNode& n2, const std::unordered_set<std::string>& required_inputs,
...
//        }
    ...
    // This assumes the direction n1 -> n2
    MLNode merge(const MLNode& n1, const MLNode& n2, const std::vector<MLNode>& all_nodes,
                 const std::vector<MLEdge>& edges) {

        return merge(n1, n2, getRequiredInputs(n1, n2, all_nodes), edges);
    }

    MLNode merge(const MLNode& n1, const MLNode& n2, const std::vector<std::shared_ptr<MLNode>>& all_nodes,
                 const std::vector<MLEdge>& edges) {
    ...
    std::vector<MLEdge> mergeEdgesNoCopy(std::vector<MLEdge>&& edges,
        ...
        for (auto& it: unmerged_edges) {
            ...
            merged_edges.push_back(MLEdge{key.first, key.second, std::move(it.second), {}});
        }

        return merged_edges;
    }

    std::vector<MLEdge> mergeEdges(std::vector<MLEdge> edges,
                                   const std::unordered_map<std::string, std::string>& name_map) {
    ...
    std::string dumpMLNode(const MLNode& n, int indent) {
        ...
        ss << "MLNode: " << n.id
           << " #sub_nodes=" << n.sub_nodes.size()
           << " #sub_edges=" << n.sub_edges.size()
           << " size=" << n.getSize()
    ...
    bool verify(const MLNode& n) {
        ...
        for (const auto& se: n.sub_edges) {
            if (!contains(ids, se.src_id) || !contains(ids, se.tgt_id)) {
                spdlog::info("@verify nodes={} subedges={}->{}", join_as_str(ids),
                             se.src_id, se.tgt_id);
                return false;
            }
        }
    ...
    bool verify(const MLGraph& g) {
        ...
        for (const auto &e: g.edges) {
            if(!contains(subnode_ids, e.src_id)) {
                spdlog::info("graph does not contains edge src {}. edge: {}->{}", e.src_id,
                             e.src_id, e.tgt_id);
                return false;
            }
            if(!contains(subnode_ids, e.tgt_id)) {
                spdlog::info("graph does not contains edge tgt {}. edge: {}->{}", e.tgt_id,
                             e.src_id, e.tgt_id);
                return false;
            }
            ...
            for (const auto &se: e.sub_edges) {
                if (!contains(src_ids, se.src_id)) {
                    spdlog::info("Edge src verification failed. {} not found. edge: {}->{} subedge: {}->{}",
                                 se.src_id, e.src_id, e.tgt_id, se.src_id, se.tgt_id);
                    return false;
                }
                if (!contains(tgt_ids, se.tgt_id)) {
                    spdlog::info("Edge tgt verification failed. {} not found. edge: {}->{} subedge: {}->{}",
    ...
    MLBGraph toBGL(const std::vector<MLNode>& nodes, const std::vector<MLEdge>& edges) {
        MLBGraph bg;
