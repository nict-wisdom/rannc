...
Deployment MLPartDecomposer::decompose(
  ...
  if (dev_mem_ > 0) {
    const auto mem_limit =
        config::Config::get().getVal<int>(config::MEM_LIMIT_GB);
    if (mem_limit > 0) {
      dev_mem_ =
          std::min(dev_mem_, (size_t)(mem_limit * 1024L * 1024L * 1024L));
    }
    const auto mem_margin =
        config::Config::get().getVal<float>(config::MEM_MARGIN);
  ...
  const int max_pipeline =
      config::Config::get().getVal<int>(config::MAX_PIPELINE);
  const int min_pipeline_bs =
      config::Config::get().getVal<int>(config::MIN_PIPELINE_BS);
  const bool coarsen_by_time =
      config::Config::get().getVal<bool>(config::COARSEN_BY_TIME);
