...
std::string toString(const IRType& type) {
  ...
  switch (type.getBaseType()) {
    ...
    case IRBaseType::LIST: {
      ss << toString(type.getListType()) << "LIST"
         << join_as_str(toString(type.getCompoundTypes()));
      return ss.str();
    }
    case IRBaseType::TUPLE: {
...
size_t IRType::getSizeInByte() const {
  switch (base_type_) {
    case IRBaseType::SCALAR:
      return getScalarSize(scalar_type_);
    case IRBaseType::TENSOR:
      return getTensorSizeInByte(tensor_elem_type_, tensor_dim_);
    case IRBaseType::LIST:
...
std::vector<IRValue> graphInputValues(
    const std::shared_ptr<IRGraph>& irGraph, bool is_param) {
...
std::ostream& operator<<(std::ostream& os, const IRGraph& graph) {
  ...
  for (const auto& in_node : graph.getNodes()) {
    ...
    os << "  " << in_node.getName() << op_type << "("
       << join_as_str(in_node.getInputNames()) << ") => "
...
IRType doGetElemInIRType(
    const IRType& type, std::vector<StepInIValue>::const_iterator iter,
    const std::vector<StepInIValue>::const_iterator& end_iter) {
  if (iter == end_iter)
    return type;

  assert(
      type.getBaseType() == IRBaseType::LIST ||
      type.getBaseType() == IRBaseType::TUPLE);
...
IRType setDimToIRType(const IRType& type, const std::vector<int64_t>& dim) {
  return IRType::createTensorType(
      type.getTensorElemType(), dim, type.requiresGrad());
}

std::vector<std::string> doGetInputNames(
  ...
  std::copy_if(
      ...
      std::back_inserter(ret_input_names), f);
...
std::vector<std::string> getGradNames(
  ...
  std::copy_if(
      names.begin(), names.end(), std::back_inserter(grad_names),
...
bool isGraphReady(
  ...
  for (const auto& in_name : graph_input_names) {
    ...
    if (!contains(avail_locs, loc))
      return false;
  }
  return true;
}

int countValueRefCount(
...
std::unordered_map<std::string, int> countSharedRefs(
...
cloneSharedInputs(const std::shared_ptr<IRGraph>& ir_graph) {
  ...
  const std::unordered_map<std::string, int> shared_no_param_inputs =
      countSharedRefs(ir_graph, no_param_inputs);
  std::vector<IRValue> param_inputs = graphParamInputValues(ir_graph);
  const std::unordered_map<std::string, int> shared_param_inputs =
      countSharedRefs(ir_graph, param_inputs);

  const std::unordered_map<std::string, int> shared_input_ref_count =
      addAll(shared_no_param_inputs, shared_param_inputs);
...
size_t calcCommBufSize(const std::shared_ptr<IRGraph>& g) {
  ...
  // When performing gradient checkpointing, we run a backward pass soon after
  ...
  // The memory for outputs is allocated by PyTorch, and the size is included in
  // the memory requirement measured by GraphProfiler. However, we still need
  // the output size to receiving gradients of outputs.
