...
namespace rannc {
    ...
    public:
        ...
        static IRType createTensorType(IRTensorElemType tensor_type, std::vector<int64_t> dim, bool requires_grad) {
            return IRType(IRBaseType::TENSOR, IRScalarType::NONE, std::move(dim), tensor_type,
        ...
        static IRType createUnknownTensorType() {
            return IRType(IRBaseType::TENSOR, IRScalarType::NONE, std::vector<int64_t>(), IRTensorElemType::UNDEF,
                          std::vector<IRType>(), false);
        }
        static IRType createStringType() {
            return IRType(IRBaseType::STRING, IRScalarType::NONE, std::vector<int64_t>(), IRTensorElemType::UNDEF,
                          std::vector<IRType>(), false);
        }
        static IRType createNoneType() {
            return IRType(IRBaseType::NONE, IRScalarType::NONE, std::vector<int64_t>(), IRTensorElemType::UNDEF,
                          std::vector<IRType>(), false);
        }
        ...
        static IRType createListType(std::vector<IRType> elem_types) {
            IRType type(IRBaseType::LIST, IRScalarType::NONE, std::vector<int64_t>(),
        ...
        static IRType createTensorListType(std::vector<IRType> elem_types) {
            IRType type(IRBaseType::LIST, IRScalarType::NONE, std::vector<int64_t>(), IRTensorElemType::UNDEF,
                          elem_types, false);
        ...
        static IRType createTupleType(std::vector<IRType> compound_types) {
            return IRType(IRBaseType::TUPLE, IRScalarType::NONE, std::vector<int64_t>(), IRTensorElemType::UNDEF,
                          std::move(compound_types), false);
        }
        ...
        **/
        ...
        static IRType createOptionalType(IRType elem_type) {
            ...
            return IRType(IRBaseType::OPTIONAL, IRScalarType::NONE, std::vector<int64_t>(), IRTensorElemType::UNDEF,
                          compound_type, false);
        }
...
//        }
        ...
        bool operator==(const IRType &rhs) const {
            if (base_type_ != rhs.base_type_) return false;

            switch (base_type_) {
                case IRBaseType::SCALAR: return scalar_type_ == rhs.scalar_type_;
                case IRBaseType::TENSOR: return tensor_dim_ == rhs.tensor_dim_ && tensor_elem_type_ == rhs.tensor_elem_type_;
                case IRBaseType::LIST: return compound_types_ == rhs.compound_types_;
                case IRBaseType::TUPLE: return compound_types_ == rhs.compound_types_;
                case IRBaseType::STRING: return compound_types_ == rhs.compound_types_;;
                case IRBaseType::FUNCTION: return func_name_ == rhs.func_name_;
                case IRBaseType::OPTIONAL: return compound_types_ == rhs.compound_types_;
                case IRBaseType::NONE: return compound_types_ == rhs.compound_types_;
            }
        }
        ...
        MSGPACK_DEFINE(base_type_, scalar_type_, tensor_dim_, tensor_elem_type_, requires_grad_, list_type_, compound_types_, list_size, func_name_);


    private:
        IRType(IRBaseType baseType, IRScalarType scalarType, std::vector<int64_t> tensorDim,
               IRTensorElemType tensorElemType, std::vector<IRType> compoundTypes,
               bool requires_grad) : base_type_(baseType), scalar_type_(scalarType),
         ...
         * Creates scalar type.
         */
        IRType(IRScalarType scalar_type): base_type_(IRBaseType::SCALAR),
        ...
        **/
        ...
        // valid if LIST, Tuple, or OPTIONAL. Note that the number of elements is always one for LIST and OPTIONAL.
    ...
    public:
        ...
        IRValue(std::string name, IRType type)
                : name_(std::move(name)), type_(std::move(type)), is_param_(false), is_batch_(false),
                is_loss_(false) {
        }
    ...
    public:
        ...
        IRNode(std::string name, std::vector<std::string> inputNames, std::vector<std::string> outputNames) :
                name_(std::move(name)), input_names_(std::move(inputNames)), output_names_(std::move(outputNames)),
                id_(generateName("node_")), is_criterion_(false), is_batch_(false) {
        }
        ...
        **/
        ...
        MSGPACK_DEFINE(name_, input_names_, output_names_, id_, is_criterion_, is_batch_);
    ...
    public:
        ...
        **/
        ...
        MSGPACK_DEFINE(name_, nodes_, values_, input_names_, output_names_, is_replicable_);
    ...
    /**
     * Get constant values in an IRGraph. The result is sorted by names of the values.
     *
    ...
    /**
     * Get parameter values in an IRGraph. The result is sorted by names of the values.
     *
     * @param irGraph Graph to search.
