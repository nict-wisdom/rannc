...
void processTensorInIValue(
    const torch::jit::IValue& ivalue,
    const std::function<void(at::Tensor)>& f) {
  if (ivalue.isTensor()) {
    f(ivalue.toTensor());
...
void setZerosToGradInIValueIfUndefined(
    torch::jit::IValue& ivalue, BufferTensorCache& buf_cache,
...
torch::jit::IValue sumGradTensorsInIValues(
  ...
  return aggregateTensorsInIValues(
      ivalues, [&buf_cache, &key](const std::vector<at::Tensor>& tensors) {
        at::Tensor grad_sum;
...
std::vector<torch::jit::IValue> getNonParamInputElems(
...
std::shared_ptr<IRGraph> insertInValueHook(
  ...
  for (const auto& n : g->getNodes()) {
    ...
    for (const auto& in_name : n.getInputNames()) {
      assert(contains(vals, in_name));
      const IRValue& in_val = vals.at(in_name);

      if (!inserted && !in_val.isParam()) {
        const std::string hook_input_name = f(in_val, n, g);
        if (!hook_input_name.empty()) {
          ...
          hook_out_names[in_name] = hook_out_name;
          new_values[hook_out_name] = IRValue(hook_out_name, in_val);
...
std::shared_ptr<IRGraph> insertOffloadingPreHooks(
  ...
  return insertInValueHook(
      g, constants, "rannc::offloadingPreHook",
      [](const IRValue& in, const IRNode& node,
...
std::shared_ptr<IRGraph> insertOffloadingPostHooks(
    const std::shared_ptr<IRGraph>& g, IValueMap& constants) {
  return insertOutValueHook(
...
void TorchDriver::createModule(
  ...
  // If you profiled a graph and ran backward, the gradients of the params still
  // remain non-zero
  for (auto& it : param_tensors_[id]) {
...
void TorchDriver::displayValue(
  ...
  if (display_comm_values_) {
    ...
    if (vals.empty()) {
      this->logger->info(
          "@rank{} {} no data to display", mpi::getRank(), prefix);
    } else {
      for (const auto& it : vals) {
        const auto& loc = it.first;
        const std::function<at::Tensor(const at::Tensor&, const IValueLocation)>
            f = [this, &prefix, &tid, count, split_index, grad_mode](
                    const at::Tensor& t, const IValueLocation& loc) {
              this->logger->info(
                  "@rank{} {} count={} tid={} split={} grad={}: {} {} {} v={}",
                  mpi::getRank(), prefix, count, toString(tid), split_index,
                  grad_mode, toString(loc), toString(toIRType(t)),
                  t.device().str(), tensorToString(t));
              return at::Tensor();
            };
...
IValueMap TorchDriver::forward(
  ...
  recordStart(
      getFuncKey("TorchDriver", "forward_copy_in", id, split_idx, grad_mode));

  IValueMap graphIn;
  std::unordered_map<std::string, std::vector<std::string>>& clone_names =
      input_clone_names_[id];
  for (const auto& in : inputs) {
    if (contains(clone_names, in.first.value_name)) {
      for (const auto& cl_name : clone_names.at(in.first.value_name)) {
        ...
        const auto cl_ivalue = cloneTensorsInIValueWithBuffer(
            in.second, ss.str(), buffer_cache_[id]);
  ...
  std::vector<torch::jit::IValue> non_param_input_elems = getNonParamInputElems(
      ir_graphs_[id], input_clone_names_[id], graphIn, true);
  auto non_param_inputs = c10::ivalue::Tuple::create(non_param_input_elems);
  std::vector<torch::jit::IValue> stack;
  stack.emplace_back(non_param_inputs);

  recordStart(getFuncKey(
      "TorchDriver", "forward_copy_param", id, split_idx, grad_mode));
...
void matchGrads(
    const torch::jit::IValue& out_iv, const torch::jit::IValue& grad_iv,
  ...
  if (out_iv.isTensor()) {
    ...
    for (size_t i = 0; i < out_iv_elems.size(); i++) {
      matchGrads(
          out_iv_elems.get(i), grad_iv_elems.get(i), out_tensors, grad_tensors);
    }
    ...
    for (size_t i = 0; i < out_iv_elems.size(); i++) {
      matchGrads(
          out_iv_elems.at(i), grad_iv_elems.at(i), out_tensors, grad_tensors);
    }
  } else {
...
IValueMap TorchDriver::backward(
    const std::string& id, const IValueMap& inputs, int split_idx) {
  ...
  logger->trace(
      "TorchDriver::backward starting. id={} split={}", id, split_idx);
  time_counter_.start(tc_key);
  ...
  recordStart(
      getFuncKey("TorchDriver", "backward_sum_ingrad", id, split_idx, false));
  ...
  for (const auto& in_name : ir_graphs_[id]->getInputNames()) {
    ...
    if (passedForBackward(type)) {
      ...
      if (contains(clone_names, in_name)) {
        ...
        inGrads[in_name] =
            sumGradTensorsInIValues(input_ivals, buffer_cache_[id], ss.str());
        ...
        const auto grad_iv = transformTensorsInIValue(
            iv, [](const at::Tensor& t) { return t.grad().contiguous(); });
  ...
  logger->trace(
      "TorchDriver::backward finished. id={} split={}", id, split_idx);
