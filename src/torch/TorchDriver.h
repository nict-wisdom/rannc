...
namespace rannc {
    ...
    public:
        ...
        /**
         * Creates a Torch module from an *IRGraph*. The *IRGraph* given as an argument is converted to *torch::jit::Graph* using *GraphConv::toTorch()*.
         *
         ...
         */
        void createModule(const std::string &id, const std::shared_ptr<rannc::IRGraph> &irGraph,
         ...
         */
        IValueMap forward(const std::string &id, const IValueMap &inputs, int split_idx);
         ...
         */
        IValueMap backward(const std::string &id, const IValueMap &inputs, int split_idx);
    ...
    private:
        ...
        std::unordered_map<std::string, std::shared_ptr<rannc::IRGraph>> clone_input_ir_graphs_;

        /**
         * Parameter tensors.
         */
        std::unordered_map<std::string, std::unordered_map<std::string, at::Tensor>> param_tensors_;
        std::unordered_map<std::string, std::vector<std::string>> ordered_param_names_;
        std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> input_clone_names_;
        std::unordered_map<std::string, std::unordered_map<std::string, std::vector<at::Tensor>>> clone_params_;

        /**
         * Script modules built from Torch IR. The key is a graph ID.
         */
        std::unordered_map<std::string, std::shared_ptr<torch::jit::Function>> functions_;
