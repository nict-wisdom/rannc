...
torch::jit::IValue processTensorInIValue(
    const torch::jit::IValue& iv,
...
torch::jit::IValue to(
    const torch::jit::IValue& iv, const torch::Device& device, bool detach,
    bool non_blocking) {
  return processTensorInIValue(
      iv, [&device, detach, non_blocking](at::Tensor t) {
        if (t.numel() == 0) {
          return t;
        }
...
torch::jit::IValue detach(const torch::jit::IValue& iv) {
  return processTensorInIValue(iv, [](at::Tensor t) { return t.detach(); });
}

torch::jit::IValue toCPU(
    const torch::jit::IValue& iv, bool detach, bool non_blocking) {
...
torch::jit::IValue toCUDAIfAvailable(
    const torch::jit::IValue& iv, bool detach, bool non_blocking) {
...
at::Tensor toCUDAIfAvailable(
    const at::Tensor& t, bool detach, bool non_blocking) {
  auto ret = t;
  if (torch::cuda::is_available()) {
...
IValueMap toCUDAIfAvailable(
    const IValueMap& iv_map, bool detach, bool non_blocking) {
  IValueMap ret;
  for (const auto& it : iv_map) {
...
torch::jit::IValue contiguous(const torch::jit::IValue& iv) {
  return processTensorInIValue(iv, [](at::Tensor t) { return t.contiguous(); });
}

torch::jit::IValue setRequiresGrad(
    const torch::jit::IValue& iv, bool requires_grad) {
...
void doFindPathsToTensorInIValue(
    const torch::jit::IValue& ivalue, PathInIValue& path,
...
std::vector<PathInIValue> findPathsToTensorInIValue(
...
torch::jit::IValue doGetElemInIValue(
    const torch::jit::IValue& ivalue,
    std::vector<StepInIValue>::const_iterator iter,
  ...
  switch (iter->type) {
    case StepTypeInIValue::LIST:
      if (ivalue.isTensorList()) {
        ...
        //                    const auto &tensor_elems =
        //                    ivalue.toTensorList()->elements();
        return tensor_elems.at(iter->index);
      } else if (ivalue.isList()) {
        const auto& ivalue_elems = ivalue.toListRef();
        return doGetElemInIValue(
            ivalue_elems.at(iter->index), iter + 1, end_iter);
      }
...
torch::jit::IValue getElemInIValue(
...
IRTensorElemType toTensorElemType(const c10::ScalarType& scalarType) {
  ...
  std::stringstream ss;
  ss << "Failed to convert c10::ScalarType to IRTensorElemType: "
...
IRType toIRType(const torch::jit::IValue& ivalue) {
  if (ivalue.isInt()) {
    ...
    return IRType::createListType(
        IRListType::FLOAT, ivalue.toDoubleList().size());
  } else if (ivalue.isBoolList()) {
    return IRType::createListType(IRListType::BOOL, ivalue.toBoolList().size());
  } else if (ivalue.isTensor()) {
    auto t = ivalue.toTensor();
    if (t.defined()) {
      return IRType::createTensorType(
          toTensorElemType(t.scalar_type()), getTensorDim(t),
          t.requires_grad());
    } else {
...
torch::jit::IValue transformTensorsInIValueWithPath(
    const torch::jit::IValue& ivalue, const IValueLocation& loc,
    const std::function<
        at::Tensor(const at::Tensor&, const IValueLocation& loc)>& f) {
  if (ivalue.isTensor()) {
    ...
    for (const auto& e : ivalue.toListRef()) {
      list.push_back(
          transformTensorsInIValueWithPath(e, createListElem(loc, idx++), f));
    }
    return list;
  } else if (ivalue.isTuple()) {
    std::vector<torch::jit::IValue> elems;
    ...
    for (const auto& e : ivalue.toTuple()->elements()) {
      elems.push_back(
          transformTensorsInIValueWithPath(e, createTupleElem(loc, idx++), f));
    }
...
torch::jit::IValue cloneTensorsInIValueWithBuffer(
    const torch::jit::IValue& ivalue, const std::string& key,
    BufferTensorCache& buffer) {
  return transformTensorsInIValueWithPath(
      ivalue, key, [&buffer](const at::Tensor& t, const IValueLocation& loc) {
...
torch::jit::IValue transformTensorsInIValue(
    const torch::jit::IValue& ivalue,
    const std::function<at::Tensor(const at::Tensor&)>& f) {
  IValueLocation loc("NA");
  return transformTensorsInIValueWithPath(
      ivalue, loc,
      [&f](const at::Tensor& t, const IValueLocation& loc) { return f(t); });
}

torch::jit::IValue transformTensorsInIValueWithPath(
    const torch::jit::IValue& ivalue, const std::string& name,
    const std::function<
        at::Tensor(const at::Tensor& t, const IValueLocation& loc)>& f) {
...
// and does not care if it is actually a batch or the first dimension represents
// the actual batch size.
at::Tensor alignTensor(
    const at::Tensor& tensor, int batch_size, bool zero_pad) {
  const auto& dim = getTensorDim(tensor);
  assert(!dim.empty());
...
torch::jit::IValue padTensorsInIValue(
    const torch::jit::IValue& ivalue, int batch_size, bool zero) {
  const std::function<at::Tensor(const at::Tensor&)> f =
      [batch_size, zero](const at::Tensor& t) {
        return padTensor(t, batch_size, zero);
      };
  return transformTensorsInIValue(ivalue, f);
}

torch::jit::IValue unpadTensorsInIValue(
    const torch::jit::IValue& ivalue, int batch_size) {
  const std::function<at::Tensor(const at::Tensor&)> f =
      [batch_size](const at::Tensor& t) { return unpadTensor(t, batch_size); };
  return transformTensorsInIValue(ivalue, f);
}

torch::jit::IValue splitBatchInIValue(
    const torch::jit::IValue& ivalue, size_t current_batch_size,
    size_t total_batch_size, size_t replica_num, bool zero_pad) {
  const std::function<at::Tensor(const at::Tensor&)> f =
      [current_batch_size, total_batch_size, replica_num,
       zero_pad](const at::Tensor& t) {
        ...
        std::unordered_map<int, std::vector<int64_t>> dist_dim =
            bs_calc.calcDistBatchDims(
                {(int64_t)total_batch_size}, dummy_ranks, 0);
...
torch::jit::IValue scaleBatchInIValue(
    const torch::jit::IValue& ivalue, size_t batch_size, size_t tgt_batch_size,
    bool zero_pad) {
  const std::function<at::Tensor(const at::Tensor&)> f =
...
torch::jit::IValue replicateTensorsInIValue(
    const torch::jit::IValue& ivalue, size_t replica_num) {
  const std::function<at::Tensor(const at::Tensor&)> f =
...
torch::jit::IValue alignTensorsInIValue(
    const torch::jit::IValue& ivalue, size_t target_batch_size, bool zero_pad) {
  const std::function<at::Tensor(const at::Tensor&)> f =
...
torch::jit::IValue scaleTensorsInIValue(
    const torch::jit::IValue& ivalue, double scale) {
  const std::function<at::Tensor(const at::Tensor&)> f =
      [scale](const at::Tensor& t) { return t.mul(scale).detach(); };
  return transformTensorsInIValue(ivalue, f);
}
...
at::Tensor sliceDistBatchTensor(
    const at::Tensor& tensor, int index, int64_t batch_size, int num) {
  const auto& dim = getTensorDim(tensor);
  ...
  const auto dist_dims =
      bs_calc.calcDistBatchDims(dim, vectorToSet(dummy_ranks), 0);
...
torch::jit::IValue sliceOrWeightTensorsInIValue(
    const torch::jit::IValue& ivalue, const std::vector<int64_t>& batch_sizes,
    int index) {
  std::function<at::Tensor(const at::Tensor&)> f =
      [&batch_sizes, index](const at::Tensor& t) {
        ...
        if (dim.empty()) {
          ...
          //                        spdlog::info("@sliceOrWeightTensorsInIValue
          //                        {} split={} scale={} loss={}",
          //                                     toString(toIRType(t)),
          //                                     index, (batch_sizes.at(index) /
          //                                     (double) sum),
          //                                     tensorToString((batch_sizes.at(index)
          //                                     / (double) sum) * t));
...
torch::jit::IValue cloneTensorsInIValue(
    const torch::jit::IValue& ivalue, const c10::Device& dev) {
  std::function<at::Tensor(const at::Tensor&)> f = [dev](const at::Tensor& t) {
    at::TensorOptions options;
    options = options.device(dev).dtype(t.scalar_type());
...
torch::jit::IValue aggregateTensorsInIValues(
    const std::vector<torch::jit::IValue>& ivalues,
...
torch::jit::IValue concatTensorsInIValues(
    const std::vector<torch::jit::IValue>& ivalues) {
  return aggregateTensorsInIValues(
      ivalues, [](const std::vector<at::Tensor>& tensors) {
        torch::NoGradGuard no_grad;
        return torch::cat(tensors);
      });
}

at::Tensor sumDistLossTensors(
    const std::vector<at::Tensor>& tensors, size_t batch_size) {
  torch::NoGradGuard no_grad;
...
torch::jit::IValue concatOrSumTensorsInIValues(
    const std::vector<torch::jit::IValue>& ivalues, size_t batch_size) {
  return aggregateTensorsInIValues(
      ivalues, [batch_size](const std::vector<at::Tensor>& tensors) {
        torch::NoGradGuard no_grad;
...
torch::jit::IValue sumTensorsInIValues(
    const std::vector<torch::jit::IValue>& ivalues) {
  return aggregateTensorsInIValues(
      ivalues, [](const std::vector<at::Tensor>& tensors) {
        assert(!tensors.empty());
...
template <typename T>
bool equalsList(
    const torch::jit::IValue& iv1, const torch::jit::IValue& iv2,
...
bool equalsTuple(
    const torch::jit::IValue& iv1, const torch::jit::IValue& iv2,
    const std::function<
        std::vector<torch::jit::IValue>(const torch::jit::IValue&)>& to_elems,
    const std::function<
        bool(const torch::jit::IValue&, const torch::jit::IValue&)>& compare) {
...
bool equals(const torch::jit::IValue& iv1, const torch::jit::IValue& iv2) {
  if (iv1.isInt() && iv2.isInt()) {
    ...
    return equalsList<int64_t>(
        iv1, iv2, [](const torch::jit::IValue& iv) { return iv.toIntList(); },
        [](const int64_t& v1, const int64_t& v2) { return v1 == v2; });
  } else if (iv1.isDoubleList() && iv2.isDoubleList()) {
    return equalsList<double>(
        iv1, iv2,
        [](const torch::jit::IValue& iv) { return iv.toDoubleList(); },
        [](const double& v1, const double& v2) { return v1 == v2; });
  } else if (iv1.isBoolList() && iv2.isBoolList()) {
    return equalsList<bool>(
        iv1, iv2, [](const torch::jit::IValue& iv) { return iv.toBoolList(); },
    ...
    return equalsList<at::Tensor>(
        iv1, iv2,
        [](const torch::jit::IValue& iv) { return iv.toTensorList(); },
        [](const at::Tensor& v1, const at::Tensor& v2) {
          return torch::equal(v1, v2);
        });
  } else if (iv1.isList() && iv2.isList()) {
    return equalsList<torch::jit::IValue>(
        iv1, iv2, [](const torch::jit::IValue& iv) { return iv.toList(); },
        [](const torch::jit::IValue& v1, const torch::jit::IValue& v2) {
          return equals(v1, v2);
        });
  } else if (iv1.isTuple() && iv2.isTuple()) {
    return equalsTuple(
        iv1, iv2,
        [](const torch::jit::IValue& iv) { return iv.toTuple()->elements(); },
        [](const torch::jit::IValue& v1, const torch::jit::IValue& v2) {
          return equals(v1, v2);
        });
...
void halfToFloat(void* half_buf, float* float_buf, int count) {
  const auto half_t =
      torch::from_blob(half_buf, {count}, c10::ScalarType::Half);
...
void floatToHalf(float* float_buf, void* half_buf, int count) {
  const auto float_t =
      torch::from_blob(float_buf, {count}, c10::ScalarType::Float);
...
bool isGraphReady(
    const std::vector<std::string>& input_names, const IValueMap& inputs) {
...
void showMem(const std::string& prefix, int rank) {
  ...
  if (torch::cuda::is_available()) {
    ...
    const DeviceStats stats =
        c10::cuda::CUDACachingAllocator::getDeviceStats(dev_id);
    spdlog::info(
        "{}: dev={} alloc={} max_alloc={} cache={}", prefix, dev_id,
        stats.allocated_bytes[0].current, stats.allocated_bytes[0].peak,
...
at::Tensor BufferTensorCache::get(const std::string& key, const IRType& type) {
  ...
  if (!contains(buf_tensors_, key) ||
      toString(types_.at(key)) != toString(type)) {
    types_[key] = type;
    buf_tensors_[key] = createBufTensor(type);

    //            spdlog::info("@createBufTensor {} {} numel={} nbytes={}", key,
    //            toString(type),
    //                         buf_tensors_[key].numel(),
    //                         buf_tensors_[key].nbytes());
  }
...
at::Tensor createBufTensor(const IRType& type) {
  const auto scalar_type =
      fromIRTensorElemTypeToScalarType(type.getTensorElemType());
  at::TensorOptions options;
  if (torch::cuda::is_available()) {
  ...
  options = options.dtype(scalar_type).requires_grad(type.requiresGrad());
...
RngState getRngState() {
  ...
  at::Generator cpu_gen;
  {
    std::lock_guard<std::mutex> lock(
        default_cpu_gen.unsafeGetGeneratorImpl()->mutex_);
  ...
  {
    std::lock_guard<std::mutex> lock(
        default_cuda_gen.unsafeGetGeneratorImpl()->mutex_);
...
void setRngState(const RngState& state) {
  ...
  {
    const auto def_gen =
        at::check_generator<at::CPUGeneratorImpl>(default_cpu_gen);
    const auto saved_gen =
        at::check_generator<at::CPUGeneratorImpl>(state.cpu_gen);

    std::lock_guard<std::mutex> lock(def_gen->mutex_);
    def_gen->set_engine(saved_gen->engine());
    def_gen->set_next_float_normal_sample(
        saved_gen->next_float_normal_sample());
    def_gen->set_next_double_normal_sample(
  ...
  auto default_cuda_gen = at::detail::getCUDAHooks().getDefaultCUDAGenerator();
  {
    ...
    const auto saved_gen =
        at::check_generator<at::CUDAGeneratorImpl>(state.cuda_gen);

    std::lock_guard<std::mutex> lock(def_gen->mutex_);

    def_gen->set_current_seed(saved_gen->current_seed());
    def_gen->set_philox_offset_per_thread(
        saved_gen->philox_offset_per_thread());
  }
}
