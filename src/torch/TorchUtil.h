...
std::string tensorToString(
    const at::Tensor& t, size_t max_elem = 20, size_t max_str_len = 100);
std::string tensorPtrToString(
    void* ptr, size_t size, c10::ScalarType datatype, size_t max_elem = 20,
...
torch::jit::IValue toCPU(
    const torch::jit::IValue& iv, bool detach, bool non_blocking = false);
torch::jit::IValue toCUDAIfAvailable(
    const torch::jit::IValue& iv, bool detach, bool non_blocking = false);
at::Tensor toCUDAIfAvailable(
    const at::Tensor& t, bool detach, bool non_blocking = false);
...
using IValueMap =
    std::unordered_map<IValueLocation, torch::jit::IValue, IValueLocationHash>;
std::vector<IValueLocation> getKeys(const IValueMap& map);

IValueMap toCPU(
    const IValueMap& iv_map, bool detach, bool non_blocking = false);
...
torch::jit::IValue splitBatchInIValue(
    ...
    size_t total__batch_size, size_t replica_num, bool zero_pad);
