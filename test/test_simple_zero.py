...
def test_small_param(init_dist, batch_size, iteration):
    ...
    common.run(models.SmallParamModel(), batch_size, iteration, enable_zero=True)


def test_fork_join(init_dist, batch_size, iteration):
    print("test_fork_join")
    common.run(models.ForkJoinModel(), batch_size, iteration, enable_zero=True)


def test_shared_param(init_dist, batch_size, iteration):
    print("test_shared_param")
    common.run(models.SharedParamModel(), batch_size, iteration, enable_zero=True)


def test_one_op(init_dist, batch_size, iteration):
    print("test_one_op")
    common.run(models.OneOpModel(), batch_size, iteration, enable_zero=True)


def test_tensor_mul(init_dist, batch_size, iteration):
    print("test_tensor_mul")
    common.run(models.TensorMulModel(), batch_size, iteration, enable_zero=True)
...
def test_loss_out(init_dist, batch_size, iteration):
    print("test_loss_out")
    common.run_loss(models.LossOutModel(), batch_size, iteration, enable_zero=True)
